...
def generate_hidden_matrix(input_size):
    ...
    return np.random.uniform((-2/input_size), (2/input_size),(9, hidden_nodes))

# Initialize weights and biases
hidden_matrix = generate_hidden_matrix(input_size)

'''
creates the input_ matrix where it gets all the input catchments and generates in the right dimensions for dot product
'''
...
#use this function to work out weighted sum using dot product of two matrixes
def get_weighted_sum_matrix(matrix_1,matrix_2):
   result = np.dot(matrix_1,matrix_2)
   return result

#function gets weighted sum of each hidden node and then works out sigmoid function on each to get 1*4 matrix 
...
def get_index_flood_predicted(x):
    ...
    return index_flood_predicted.item()

#gets the output delta needed to update the weight values 
def get_output_delta(index_flood_actual,x,omega,upsilon):
    #x is the predicted index flood
    output_delta = ((index_flood_actual - x)+(omega*upsilon))*(x*(1-x))
    return output_delta
...
def get_hidden_delta(output_delta, hidden_sigmoid_matrix, output_matrix):
    # Extract weights corresponding to hidden nodes
...
def anneal(p_start, p_end, x, epochs):
    p = p_start + (p_end - p_start) * (1 / (1 + np.exp(10 - 20 * x / epochs)))
    return p

# MSE history
mse_history_validation = []

#learning parameter history
p_history = []
...
def calculate_omega(hidden_matrix, output_matrix):
    sum_squares = 0
    n = hidden_matrix.size + output_matrix.size
    sum_squares = np.sum(np.square(hidden_matrix[1:])) + np.sum(np.square(output_matrix[1:]))
    omega = (1/(2*n))* sum_squares
    return omega

def calculate_upsilon(p,epoch):
    upsilon = 1/(p*epoch)
    return upsilon
...
for epoch in range(epochs):
    ...
    p = anneal(p_start, p_end, epoch, epochs)
    p_history.append(p)
    print(f"Epoch {epoch + 1}: Learning Rate = {p}")
    for i in range(len(df)):
        ...
        omega = calculate_omega(hidden_matrix,output_matrix)
        upsilon = calculate_upsilon(p,epoch+1)
        output_delta = get_output_delta(index_flood_actual,index_flood_predicted,omega,upsilon)
...
plt.plot(epochs_validation, mse_history_validation, label='MSE Validation')
plt.xlabel('Epoch')
plt.ylabel('MSE')
plt.title('MSE over Epochs - Validation Data Anneal with weight decay')
plt.legend()
plt.show()

# Plot learning parameter history
plt.figure(figsize=(10, 6))
plt.plot(range(1, epochs + 1), p_history, label='Learning parameter')
plt.xlabel('Epoch')
plt.ylabel('Learning parameter')
plt.title('Learning parameter over Epochs')
plt.legend()
plt.show()
...
plt.title('Predicted vs Actual - Test Data Anneal with weight decay')
plt.legend()
plt.show()
